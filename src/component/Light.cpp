...
void PointLight::set_attenuation(float linear, float quadratic) {
  ...
  CORE_ASERT(quadratic > 0,
             "The quadratic attenuation factor must be positive ...");
...
float PointLight::get_attenuation(float distance) const {
  ...
  CORE_ASERT(distance >= 0.0f,
             "Distance to the light source cannot be negative ...");

  return distance > 1e9 ? 0
                        : 1 / (1.0 + m_linear * distance +
                               m_quadratic * distance * distance);
}

void Spotlight::set_cutoff(float range, float inner_cutoff,
                           float outer_cutoff) {
...
float Spotlight::get_inner_cosine() const {
  return glm::cos(glm::radians(m_inner_cutoff));
}

float Spotlight::get_outer_cosine() const {
  return glm::cos(glm::radians(m_outer_cutoff));
}
