 ...
 * @LastEditors  : xietao
 * @LastEditTime : 2022-12-15 22:52:14
...
class Event {
 ...
 private:
  ...
  /**
   * @description : 当鼠标移动时 调用回调 并将鼠标状态写入输入事件中
   * @param        {GLFWwindow*} window
   * @param        {double} xpos
   * @param        {double} ypos
   * @return       {*}
  ...
  /**
   * @description : 当鼠标按压时 调用回调 并将鼠标状态写入输入事件中
   * @param        {GLFWwindow*} window
   * @param        {int} button
   * @param        {int} action
   * @param        {int} mods
  ...
  /**
   * @description : 当键盘按压时 调用回调 且将键盘状态写入输入事件中
