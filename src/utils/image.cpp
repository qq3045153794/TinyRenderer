...
#include "stb_image.h"
namespace utils {
Image::Image(const std::string& img_path, bool flip)
    : m_width(0), m_height(0), n_channels(0) {
  ...
  if (m_is_hdr) {
    float* buffer =
        stbi_loadf(img_path.c_str(), &m_width, &m_height, &n_channels, 0);
    ...
    CORE_DEBUG("luminance max {}, luminance min {}", luminance_max,
               luminance_min);
...
bool Image::is_hdr() const { return m_is_hdr; }

Image::~Image() { stbi_image_free(m_buffer); }

GLuint Image::get_width() const { return static_cast<GLuint>(m_width); }

GLuint Image::get_height() const { return static_cast<GLuint>(m_height); }
...
uint8_t* Image::get_buffer() const { return m_buffer; }
