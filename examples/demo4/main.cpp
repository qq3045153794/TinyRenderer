...
void init() {
  ...
  camera_p = new gl_cameras::Camera(
      45.0f, static_cast<float>(SCREEN_WIDTH) / static_cast<float>(SCREEN_HEIGHT), 0.1f, 1000.0f);
  camera_p->look_at(glm::vec3(0.0f, 0.0f, 3.0f), glm::vec3(0.0f, 0.0f, 0.0f),
                    glm::vec3(0.0f, 1.0f, 0.0f));

  ResourceManager::get_shader("anim").set_matrix4("projection", camera_p->get_projection_mat(),
                                                  true);
}
int main() {
  glfwInit();
